 ...
 * Built 2026-04-29T15:08:50.513Z
...
(function bootstrap() {
    ...
    try {
        ...
        _pill.type = 'button';
        _pill.textContent = '📱';
        _pill.title = 'CuiPhone build=2026-04-29T15-08-50-513Z';
        _pill.style.cssText = [
...
;(function wireSettingsPanel(){
    ...
    function attach(){
        ...
        if (importBtn) importBtn.addEventListener('click', () => {
            // Pull <bqb> entries that the worldbook injected into thread or sticker-list.
            // We crawl the sticker-list panel which the original phone.js populates from
            // worldbook data on import.
            const stickers = [];
            root.querySelectorAll('.sticker-btn').forEach(btn => {
                const desc = (btn.textContent || '').trim();
                if (desc) stickers.push({ desc, icon: '😶', ts: Date.now() });
            });
            const existing = readStickers();
            const seen = new Set(existing.map(s => s.desc));
            const merged = existing.concat(stickers.filter(s => !seen.has(s.desc)));
            saveStickers(merged);
            renderStickerRepo();
            importBtn.textContent = '✓ 已同步 ' + stickers.length;
            setTimeout(() => { importBtn.textContent = '从世界书同步'; }, 1500);
        });
