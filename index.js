 ...
 * Built 2026-05-01T07:54:04.315Z
...
(function bootstrap() {
    ...
    try {
        ...
        _pill.type = 'button';
        _pill.textContent = '📱';
        _pill.title = 'CuiPhone build=2026-05-01T07-54-04-315Z';
        _pill.style.cssText = [
...
;(function wirePlusSheet(){
    ...
    function attach(){
        ...
        if (sendBtn) sendBtn.addEventListener('click', function(){
            ...
            var who;
            if (roomKind === '群聊' || (info.others && info.others.length > 1)){
                who = '群里非{{user}}的角色（候选：' + (info.others.join('、') || '群成员') + '）';
            } else {
                who = (info.others && info.others[0]) || roomName;
            }
            var lines = picked.map(function(it, i){ return (i+1) + '. ' + it.text; }).join('\n');
            var payload = '（系统）我（{{user}}）刚刚在 KKT 给「' + roomName + '」发了 ' + picked.length + ' 条消息：\n'
                        + lines + '\n'
                        + '请你以「' + who + '」的身份按当前剧情自然回应。';
