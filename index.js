 ...
 * Built 2026-04-29T15:26:41.494Z
...
(function bootstrap() {
    ...
    try {
        ...
        _pill.type = 'button';
        _pill.textContent = '📱';
        _pill.title = 'CuiPhone build=2026-04-29T15-26-41-495Z';
        _pill.style.cssText = [
...
;(function wireMusicAndActions(){
    const PLAY_KEY = 'cuiphone:music_playlist';
    const STATE_KEY = 'cuiphone:music_state';
    const FAKE_DURATION = 180; // seconds, for URL-less tracks
    function readPlaylist(){
        try { return JSON.parse(localStorage.getItem(PLAY_KEY) || '[]') || []; }
        catch(_){ return []; }
    }
    function savePlaylist(arr){ try { localStorage.setItem(PLAY_KEY, JSON.stringify(arr)); } catch(_){} }
    function readState(){
        try { return JSON.parse(localStorage.getItem(STATE_KEY) || 'null') || { idx:0, playing:false, t:0 }; }
        catch(_){ return { idx:0, playing:false, t:0 }; }
    }
    ...
    function attach(){
        const root = (typeof document !== 'undefined') && document.getElementById('cui-phone-root');
        if (!root) { setTimeout(attach, 200); return; }
        if (root.dataset.cuiMusicWired === '1') return;
        // Wait for the music bar + action dock to exist (they were injected
        // into phone.html, but phone.js may not have mounted root yet).
        if (!root.querySelector('#cuiMusicBar') || !root.querySelector('.cui-action-dock')) {
            setTimeout(attach, 200); return;
        }
        root.dataset.cuiMusicWired = '1';

        // ===== Music player =====
        const bar = root.querySelector('#cuiMusicBar');
        const audio = root.querySelector('#cuiMusicAudio');
        const titleEl = root.querySelector('#cuiMusicTitle');
        const fillEl = root.querySelector('#cuiMusicProgress');
        const playBtn = root.querySelector('#cuiMusicPlay');
        const prevBtn = root.querySelector('#cuiMusicPrev');
        const nextBtn = root.querySelector('#cuiMusicNext');
        const closeBtn = root.querySelector('#cuiMusicClose');
        const artBtn = root.querySelector('#cuiMusicArt');
        ...
        function currentTrack(){
            const arr = readPlaylist();
            if (!arr.length) return null;
            if (state.idx >= arr.length) state.idx = 0;
            return arr[state.idx] || null;
        }
        function renderMeta(){
            const t = currentTrack();
            if (titleEl) titleEl.textContent = t ? (t.title || '未命名曲目') : '播放列表为空';
        }
        ...
        function startFake(fromSec){
            ...
            fakeTimer = setInterval(() => {
                ...
                const dur = FAKE_DURATION;
                if (elapsed >= dur) {
                    state.t = 0; saveState(state);
                    next();
                    return;
                }
                state.t = elapsed; saveState(state);
        ...
        function play(){
            const t = currentTrack();
            if (!t) { state.playing = false; saveState(state); updatePlayBtn(); return; }
            state.playing = true; saveState(state);
            updatePlayBtn();
            if (t.url && audio) {
                try {
                    if (!audio.src) audio.src = t.url;
                    const p = audio.play();
                    if (p && p.catch) p.catch(() => { /* autoplay block — fall back to fake */ startFake(state.t || 0); });
                } catch(_){ startFake(state.t || 0); }
            } else {
                startFake(state.t || 0);
            }
        }
        function pause(){
            state.playing = false; saveState(state);
            updatePlayBtn();
            stopFake();
        ...
        function prev(){
            const arr = readPlaylist();
            if (!arr.length) return;
            state.idx = (state.idx - 1 + arr.length) % arr.length;
            state.t = 0; saveState(state);
            loadCurrent();
            if (state.playing) play();
        }
        ...
        if (audio) {
            audio.addEventListener('timeupdate', () => {
                ...
                setProgress(pct);
                state.t = audio.currentTime; saveState(state);
            });
            audio.addEventListener('ended', () => { state.t = 0; saveState(state); next(); });
            audio.addEventListener('error', () => {
                // Fall back to fake-progress mode if URL fails to load.
                if (state.playing) startFake(0);
            });
        }
        if (playBtn) playBtn.addEventListener('click', toggle);
        if (prevBtn) prevBtn.addEventListener('click', prev);
        if (nextBtn) nextBtn.addEventListener('click', next);
        if (closeBtn) closeBtn.addEventListener('click', () => { pause(); bar.hidden = true; });
        if (artBtn) artBtn.addEventListener('click', () => {
            // Open settings → music tab
            const views = root.querySelectorAll('.view');
            views.forEach(v => v.classList.toggle('active', v.dataset.view === 'settings'));
            const tabs = root.querySelectorAll('.settings-tab');
            tabs.forEach(b => b.classList.toggle('active', b.dataset.settingsTab === 'music'));
            root.querySelectorAll('.settings-panel').forEach(p => p.classList.toggle('active', p.dataset.settingsPanel === 'music'));
            renderMusicList();
        });
        ...
        loadCurrent();
        updatePlayBtn();
        // Don't auto-resume audio on load (autoplay blocked anyway). Just show last state.
        if (state.playing) {
            // Treat as paused after reload, but keep idx/t.
            state.playing = false; saveState(state); updatePlayBtn();
        }

        // ===== Music settings tab =====
        const musicList = root.querySelector('#musicList');
        const addTitle = root.querySelector('#musicAddTitle');
        const addUrl = root.querySelector('#musicAddUrl');
        const addBtn = root.querySelector('#musicAddBtn');
        const showBarBtn = root.querySelector('#musicShowBarBtn');
        function renderMusicList(){
            if (!musicList) return;
            const arr = readPlaylist();
            if (!arr.length) {
                musicList.innerHTML = '<div class="sticker-empty">还没有曲目 · 在下面添加一条试试</div>';
                return;
            }
            musicList.innerHTML = arr.map((s, i) => {
                const isCur = (i === state.idx);
                const tag = s.url ? '直链' : '虚拟';
                return '<div class="sticker-repo-item" data-idx="' + i + '">'
                     ...
                     + '</div>'
                     + '<div class="sticker-repo-actions">'
                     + '<button class="settings-btn" data-music-play="' + i + '">' + (isCur && state.playing ? '⏸' : '▶') + '</button>'
        ...
        if (addBtn) addBtn.addEventListener('click', () => {
            ...
            renderMusicList();
            // If this was the first track, load it.
            if (arr.length === 1) { state.idx = 0; saveState(state); loadCurrent(); }
        });
        if (showBarBtn) showBarBtn.addEventListener('click', () => {
            bar.hidden = false;
            renderMeta();
            showBarBtn.textContent = '✓ 已显示';
            setTimeout(() => { showBarBtn.textContent = '显示播放器'; }, 1200);
        });
        if (musicList) musicList.addEventListener('click', e => {
            const playBtn2 = e.target.closest && e.target.closest('[data-music-play]');
            const delBtn = e.target.closest && e.target.closest('[data-music-del]');
            if (delBtn) {
                const i = parseInt(delBtn.dataset.musicDel, 10);
                const arr = readPlaylist();
                arr.splice(i, 1);
                savePlaylist(arr);
                if (state.idx >= arr.length) state.idx = 0;
                saveState(state);
                renderMusicList(); loadCurrent();
                return;
            }
            if (playBtn2) {
                const i = parseInt(playBtn2.dataset.musicPlay, 10);
                if (i === state.idx && state.playing) { pause(); }
                else {
                    state.idx = i; state.t = 0; saveState(state);
                    loadCurrent();
                    bar.hidden = false;
                    play();
                }
        ...
        // Re-render music list whenever the music tab becomes active.
        root.querySelectorAll('.settings-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                if (btn.dataset.settingsTab === 'music') renderMusicList();
            });
        });

        // ===== Action dock =====
        const dock = root.querySelector('.cui-action-dock');
        if (dock) {
            dock.addEventListener('click', e => {
                const t = e.target.closest && e.target.closest('[data-cui-action]');
                if (!t) return;
                const action = t.dataset.cuiAction;
                const CP = window.CuiPhone || {};
                if (action === 'call') {
                    // Switch to KKT and inject a call-out bubble via state.threads.
                    if (CP.switchView) CP.switchView('kkt');
                    const state2 = CP.state || {};
                    const rooms = state2.rooms || [];
                    let roomId = state2.currentRoom || (rooms[0] && rooms[0].id) || '';
                    if (!roomId && CP.renderChatList) CP.renderChatList();
                    if (roomId) {
                        state2.currentRoom = roomId;
                        state2.threads = state2.threads || {};
                        state2.threads[roomId] = state2.threads[roomId] || [];
                        const now = new Date();
                        const hh = String(now.getHours()).padStart(2,'0');
                        const mm = String(now.getMinutes()).padStart(2,'0');
                        state2.threads[roomId].push({
                            side: 'me', name: '我',
                            text: '[语音通话 00:00]',
                            time: hh + ':' + mm
                        });
                        if (CP.switchKktPanel) CP.switchKktPanel('thread');
                        if (CP.renderThread) CP.renderThread(roomId);
                    } else {
                        alert('还没有 KKT 聊天室，先从世界书同步消息。');
                    }
                } else if (action === 'ins') {
                    if (CP.switchView) CP.switchView('ins');
                    setTimeout(() => {
                        const ob = root.querySelector('#openComposerBtn');
                        if (ob) ob.click();
                    }, 80);
                    ...
                    setTimeout(() => {
                        ...
                        if (ob) ob.click();
                        setTimeout(() => {
                            const tab = root.querySelector('[data-compose="story"]');
                            if (tab) tab.click();
                        }, 60);
                    }, 80);
                } else if (action === 'multi') {
                    const sheet = root.querySelector('#cuiMultiSheet');
                    if (sheet) sheet.hidden = false;
                } else if (action === 'music') {
                    bar.hidden = !bar.hidden;
                    if (!bar.hidden) renderMeta();
                }
            });
        }
        ...
        // ===== Multi-send sheet =====
        const sheet = root.querySelector('#cuiMultiSheet');
        const mtext = root.querySelector('#cuiMultiText');
        const mint = root.querySelector('#cuiMultiInterval');
        const msend = root.querySelector('#cuiMultiSend');
        const mclose = root.querySelector('#cuiMultiClose');
        const mcancel = root.querySelector('#cuiMultiCancel');
        let multiBusy = false;
        let multiCancelled = false;
        function closeSheet(){ if (sheet) sheet.hidden = true; }
        if (mclose) mclose.addEventListener('click', () => { multiCancelled = true; closeSheet(); });
        if (mcancel) mcancel.addEventListener('click', () => { multiCancelled = true; closeSheet(); });
        if (sheet) sheet.addEventListener('click', e => { if (e.target === sheet) { multiCancelled = true; closeSheet(); } });
        if (msend) msend.addEventListener('click', async () => {
            if (multiBusy) return;
            const lines = (mtext.value || '').split(/\n+/).map(s => s.trim()).filter(Boolean);
            if (!lines.length) {
                msend.textContent = '没有内容';
                setTimeout(() => { msend.textContent = '发送'; }, 1200);
                return;
            }
            const interval = Math.max(0, Math.min(5000, parseInt(mint.value, 10) || 600));
            // Make sure we're in KKT thread view.
            const CP = window.CuiPhone || {};
            if (CP.switchView) CP.switchView('kkt');
            if (CP.switchKktPanel) CP.switchKktPanel('thread');
            const input = root.querySelector('#kktInput');
            const sendBtn = root.querySelector('#sendKktBtn');
            if (!input || !sendBtn) {
                msend.textContent = '进 KKT';
                setTimeout(() => { msend.textContent = '发送'; }, 1500);
                return;
            }
            multiBusy = true;
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                msend.textContent = '发送中 ' + (i+1) + '/' + lines.length;
                input.value = lines[i];
                input.dispatchEvent(new Event('input', { bubbles: true }));
                sendBtn.click();
                if (i < lines.length - 1) {
            ...
            multiBusy = false;
            msend.textContent = multiCancelled ? '已取消' : '✓ 已发 ' + lines.length;
            setTimeout(() => { msend.textContent = '发送'; if (!multiCancelled) { mtext.value = ''; closeSheet(); } }, 900);
        });
    }
    if (typeof window !== 'undefined') {
        if (typeof document !== 'undefined' && document.readyState !== 'loading') attach();
