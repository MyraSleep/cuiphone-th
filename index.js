 ...
 * Built 2026-04-29T16:02:42.758Z
...
(function bootstrap() {
    ...
    try {
        ...
        _pill.type = 'button';
        _pill.textContent = '📱';
        _pill.title = 'CuiPhone build=2026-04-29T16-02-42-758Z';
        _pill.style.cssText = [
...
;(function wireCuiAI(){
    // Expose a single sender that uses ST's /send + /trigger combo.
    // Escape the | character in user content so it doesn't break the chained slash.
    function safe(s){ return String(s == null ? '' : s).replace(/|/g, ' '); }
    function send(text){
        var cmd = '/send ' + safe(text) + ' |/trigger';
        try {
        ...
        } catch(err){ console.warn('[CuiPhone] triggerSlash failed', err); }
